...
const CustomersFormSchema = z.object({
    ...
    image_url: z.string({
        invalid_type_error: 'Please write a valid image path.',
    }),
});
...
export async function createCustomer(
    state: CustomerState, // Ensure state is first
    formData: FormData
...
export async function updateCustomer(
    ...
    try {
        await sql`
          UPDATE customers
          SET name = ${name}, email = ${email}, image_url = ${url}
          WHERE id = ${id}
        `;
    } catch (error) {
        console.error(error);
